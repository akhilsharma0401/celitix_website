...
const Pricing = () => {
  ...
  const fetchPricing = async (type, market, currency) => {
    try {
      ...
      const callingData =
        pricingRes?.data?.callingData?.length > 0
  ...
  return (
    <>
          ...
          <div className="flex flex-col bg-gradient-to-br from-white via-blue-50 to-blue-100 rounded-3xl shadow-xl p-2 md:p-12 w-full lg:w-3/4  transition-all duration-300">
            <div className="text-center">
              ...
              <div className="flex flex-wrap items-center justify-center gap-4 mb-6 w-full">
                <select
                  className="bg-white border w-full md:w-56 border-blue-300 text-blue-800 font-medium rounded-lg px-4 py-2 shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-400 transition"
                  value={channel}
                  onChange={(e) => {
      ...
      {/* 2rd */}
      <section className="bg-[#f7ebfc] py-8 md:py-5 lg:pb-25">
        {channel === "SMS" &&
          market !== "India" &&
          (apiData?.length > 0 ? (
            <>
                    ...
                    <tbody>
                      {currentRows.map((row) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-right font-semibold text-sky-700 truncate">
                            {currency === "inr"
                              ? `₹ ${(row.priceUSD * rates.inr).toFixed(2)}`
                              : `$ ${row.priceUSD.toFixed(5)}`}
                          </td>
                        </tr>
                      ))}
              ...
              <div className="max-w-xl mx-auto mt-10">
                <div className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm shadow-sm px-8 py-12 text-center">
                  ...
                  <p className="mt-2 text-slate-500">
                    We couldn’t find any SMS operator-wise pricing information
                    for{" "}
